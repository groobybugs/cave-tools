...
const CAVE_TOOLS_BLOCK = [
  ...
  '- Use `cave__bash` instead of the built-in shell tool for commands. Optimized drop-in replacement: tries RTK rewriting when available, then structured JSON/XML extraction and line budgets. Max timeout 10min.',
...
const ZCODE_CAVE_TOOLS_SKILL = [
  ...
  '- Use `cave__bash` instead of built-in shell execution for commands. Max timeout 10min; for anything longer use `cave__bash_start` (detached, returns a jobId) and poll `cave__bash_status` (`wait` up to 60s), stop with `cave__bash_stop`.',
  '- Do not double-wrap: never run `rtk <cmd>` inside `cave__bash`; pass the raw command.',
  ...
  '## Fallbacks',
  '',
  'Use built-in tools only when Cave Tools is unavailable or when a task needs streaming monitors, interactive stdin, or host-specific behavior Cave Tools cannot provide.',
  MARKER_END,
  '',
].join('\n');
